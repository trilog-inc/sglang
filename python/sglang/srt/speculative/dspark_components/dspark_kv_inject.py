        ...
        with context, fp8_gemm_runner_backend_context(self.fp8_gemm_backend):
            ...
            if hasattr(pool, "set_swa_key_buffer_radix_fused_norm_rope"):
                if target_hidden_is_projected:
                    raise RuntimeError(
                        "Pre-gather target-hidden projection is not supported by "
                        "the DSpark MLA KV injection path."
                    )
                self._inject_mla(
                    pool=pool,
    ...
    def _unified_inject_loc(
        ...
        *,
        ...
        if state_slot is None:
            raise RuntimeError(
                "unified_kv target-hidden injection requires per-token state_slot."
            )
        ...
        if final_pos is not None:
            keep = pos > (final_pos.to(torch.int64) - window)
            ...
            if is_unified_kv_triton():
                inject_layout = build_unified_commit_inject_layout(
                    req_pool_indices=batch.req_pool_indices.to(
                        device=self.draft_device, non_blocking=True
                    ),
                    prefix_lens=prefix_lens.to(
                        device=self.draft_device, non_blocking=True
                    ),
                    block_pos_offsets=self._block_pos_offsets[:stride].to(
                        device=self.draft_device, non_blocking=True
                    ),
                    commit_lens=commit_lens.to(
                        device=self.draft_device, non_blocking=True
                ...
                inject_layout = None
            if inject_layout is not None:
                with torch.inference_mode():
                    self.draft_model.write_target_hidden_kv(
                        main_hidden=hidden.to(
                            device=self.draft_device, non_blocking=True
