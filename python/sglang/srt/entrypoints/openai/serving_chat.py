...
class OpenAIServingChat(OpenAIServingBase):
    ...
    def __init__(
        ...
        # Values: "dsv32", "dsv4", "dsv41", or custom values set by subclass.
        # None selects the default tokenizer chat-template path.
    ...
    def _resolve_dsv41_reasoning_effort(self, value: Any) -> Union[str, int]:
        """Resolve V4.1 reasoning effort, warning once for unsupported values."""
    ...
    def _convert_to_internal_request(
        ...
        elif is_multimodal and self.chat_encoding_spec == "dsv41":
            prompt_kwargs = {"input_ids": processed_messages.prompt_ids}
        elif is_multimodal:
    ...
    def _apply_jinja_template(
        ...
        if prompt_ids is not None:
            ...
            if is_dsv41:
                # The V4.1 encoder consumes OpenAI content-part lists itself so it
                # can replace image parts with placeholders and preserve their order.
                for msg in messages:
                    if msg.get("content") is None:
                        msg["content"] = ""
            elif not dsv4_with_images:
            ...
            if messages[0]["role"] != "system" and (request.tools or not is_dsv41):
                # V4.1 renders an empty system token, so only add a system message
                # when it is needed to host tools. Older DeepSeek encoders keep
                # their existing unconditional empty system message.
            ...
            if self.chat_encoding_spec == "dsv4":
                # V4 encoder accepts low/high/max (or None).
                ...
                real_input, media_data = encoding_dsv41.encode_messages(
                    messages,
                    thinking_mode=thinking_mode,
                ...
                if media_data["images"]:
                    ...
                    for record in media_data["images"]:
                        url = record.get("url") or record.get("data")
                        if not url:
                            raise ValueError("DeepSeek-V4.1 image has no URL or data")
                        image_data.append(ImageData(url=url))
                    tokenizer = self.tokenizer_manager.tokenizer
