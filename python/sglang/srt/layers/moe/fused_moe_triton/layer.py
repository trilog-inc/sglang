    ...
    def weight_loader(
        ...
        if (
            ...
            if isinstance(self.quant_method, KTEPWrapperMethod):
                logical_ids = self.quant_method.gpu_index_to_logical.to(
                    device=loaded_weight.device
                )
                loaded_weight = loaded_weight.index_select(0, logical_ids)
                # The all-CPU case retains one zero-filled dummy GPU slot so
                # backend post-processing can initialize normally.
                if loaded_weight.shape[0] == 0:
                    return
            if "bias" in weight_name:
                dim1 = loaded_weight.shape[1]
                param.data[: loaded_weight.shape[0], :dim1].copy_(loaded_weight)
            else:
                dim1 = loaded_weight.shape[1]
                dim2 = loaded_weight.shape[2]
                param.data[
                    : loaded_weight.shape[0], :dim1, :dim2
                ].copy_(loaded_weight)
            return
        ...
        if global_expert_location_metadata is None:
            if not getattr(param, "_sglang_require_global_experts", False):
                ...
                if expert_id == -1:
                    return
            if isinstance(self.quant_method, KTEPWrapperMethod):
                expert_id = self.quant_method.gpu_weight_index(expert_id)
