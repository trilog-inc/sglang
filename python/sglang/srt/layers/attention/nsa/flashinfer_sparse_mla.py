...
def get_flashinfer_sparse_mla_op() -> Callable:
    try:
        ...
        raise RuntimeError(
            "flashinfer_sparse_mla requires FlashInfer's SM120 sparse MLA API. "
            "Install FlashInfer >=0.6.14rc1/nightly or build from the validated "
            "SM120 source ref with matching flashinfer-cubin artifacts."
        ) from exc
    ...
    if missing:
        raise RuntimeError(
            "flashinfer_sparse_mla requires FlashInfer's SM120 sparse MLA API; "
            f"the installed API is missing arguments {missing}."
        )
...
def flashinfer_sparse_mla_forward(
    ...
    *,
    ...
    if kv_cache_dim != 656:
        raise ValueError(
            "flashinfer_sparse_mla requires the 656-byte packed FP8 NSA KV "
