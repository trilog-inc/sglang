...
def _dspark_swa_page_index_alignment(block_size: int) -> int:
    """Choose a DSpark index width instantiated by the active SM120 backend."""
    ...
    raise ValueError(
        "DSpark sparse-attention width exceeds the FlashInfer SM120 envelope: "
        f"required_topk={required_topk}, supported={_SM120_FLASHINFER_DSV4_TOPKS}. "
        "Set SGLANG_SM120_FLASHMLA_BACKEND=triton for larger draft blocks."
    )
...
class DeepseekV4AttnBackend(
        ...
        **_,
        ...
        if isinstance(core_attn_metadata, DSV4AttnMetadata):
            ...
            if self.is_dspark_draft and self.cuda_capability[0] == 8:
                if compress_ratio != 0:
                    raise RuntimeError(
                        "The SM8x DSpark fallback supports SWA-only draft layers, "
                        f"got compress_ratio={compress_ratio}."
                    )
    ...
    def _forward_dspark_sm8x_swa(
        ...
        *,
        ...
        """Run the remote SM8x draft's 128-token SWA in BF16 tensor cores."""
        if q.ndim == 4:
            q = q.squeeze(1)
        if indices.ndim == 3:
