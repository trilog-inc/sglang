...
class ServerArgs:
    ...
    kt_numa_nodes: A[
        Optional[List[int]],
        "[ktransformers parameter] Explicit NUMA node ids for KT thread pools. Defaults to sequential nodes.",
        NS("exec.moe"),
    ] = None
    ...
    kt_gpu_experts_ratio: A[
        Optional[float],
        "[ktransformers parameter] Fraction of all routed-expert weights retained on GPU. Overrides --kt-num-gpu-experts.",
        NS("exec.moe"),
    ] = None
    ...
    kt_mxfp4_amx_min_tokens_per_expert: A[
        int,
    ...
    def _apply_cuda_graph_compatibility(self):
        ...
        if self.kt_weight_path is not None:
            decode_locked = (Phase.DECODE, "backend") in self._cuda_graph_config_locked
            decode_backend = self.cuda_graph_config.decode.backend
            if decode_backend == Backend.FULL:
                if decode_locked:
                    raise ValueError(
                        "--kt-weight-path is incompatible with a full decode CUDA "
                        "graph because KT executes host AMX work during each MoE "
                        "layer. Use --cuda-graph-backend-decode breakable or "
                        "--disable-decode-cuda-graph."
                    )
                logger.info(
                    "Using breakable decode CUDA graph for KT CPU/GPU expert offload."
                )
                self.cuda_graph_config.decode.backend = Backend.BREAKABLE
            elif decode_backend not in (Backend.BREAKABLE, Backend.DISABLED):
                raise ValueError(
                    "--kt-weight-path requires a breakable or disabled decode "
                    f"CUDA graph, got {decode_backend!r}."
                )
