...
For a single 96 GB RTX PRO 6000 (SM120), the KT hybrid path keeps a hot
subset of the target's routed experts in FlashInfer and executes the rest on
the CPU from the checkpoint's native packed MXFP4 weights. KT materializes
only the compact CPU complement, rather than duplicating GPU-resident experts
in system memory. The DSpark draft
layer remains GPU-only. No AMXINT4 conversion or second draft checkpoint is
used:
...
Build KT-Kernel with `CPUINFER_ENABLE_AMX=ON`,
`CPUINFER_ENABLE_AVX512_BF16=ON`, and `CPUINFER_CUDA_ARCHS=120`. The CPU must
advertise both `amx_tile` and `amx_bf16`. The native backend expands only the
current 32-value E2M1 tile, uses AMX-BF16 for sufficiently populated experts,
and applies the original UE8M0 scales in FP32. Small expert batches stay on the
AVX512-BF16 tail kernel; tune the crossover with
`--kt-mxfp4-amx-min-tokens-per-expert` after profiling.

`--kt-num-gpu-experts 96` is a starting point, not a universal maximum. Lower
it if graph capture or long-context KV allocation runs out of VRAM; increase it
until the target CPU and GPU expert phases finish at roughly the same time. A
frequency profile can replace uniform placement using
`--kt-expert-placement-strategy frequency --init-expert-location counts.pt`.
SGLang automatically changes the target's default full decode graph to a
breakable graph while retaining a full graph for the GPU-only draft. The helper script
`scripts/launch_dsv4_flash_0731_dspark_kt_sm120.sh` validates CUDA 13.3, SM120,
and AMX, builds KT-Kernel, and launches the same recipe.

On the 4×GB300 topology above, keep `--mem-fraction-static 0.90` to leave enough headroom for the batch-256 verify graph. The first cold start can take 10–15 minutes while FlashInfer autotunes and SGLang captures the draft and verify graphs; later starts reuse the cache. That path is verified end-to-end on 4×GB300 with SGLang v0.5.16.
